/*公共前后台数据*/
var homeroot = getRootPath() + '/';
homeurl = (typeof homeurl == "string") ? (homeurl ? homeurl : homeroot) : homeroot;
...
var Layer = {
    init: function(url, divw, divh, vLeft, vTop) {
        ...
        $("#-phpsys-window-bg-").animate({
            opacity: "0.8"
        }, "normal"); //设置透明度
        $("#-phpsys-window-box-").show(); //弹出内容层
        if (height >= 527) {
            $("#-phpsys-window-content-").css({
    ...
    over: function() {
        ...
        $("#-phpsys-window-box-").fadeOut("slow", function() {
            $(this).remove();
        });
    }
}
